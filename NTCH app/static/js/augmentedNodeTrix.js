function AugmentedNodeTrix(chartContainerID)
{
    ...
    chart.applyAuthorTopic = function(index)
    {
         if (typeof index === 'undefined')
         {
            for( var i = 0 ; i < _piecewiseDatasetMatrix.length;  ++i )
            {
                // _chart[i].applyAuthorTopic();
                similarityType = 'authortopic';
                _chart[i].similarityMatrices(similarityType);
            }
         }
         else
         {
            // _chart[index].applyAuthorTopic();
         }
         return chart;
    }
