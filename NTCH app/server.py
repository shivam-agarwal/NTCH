...
def fooBeforeCluster():
	...
	f1 = open("clusterstats_condensed2.txt", 'a')
	...
	if clusteringType == "louvain":
		...
		for i in range(2,d+1):
			# To avoid unnecessary calls for other number of clusters for generating images of keim c3 comm
...
def newGNScore(U,W,c):
	...
	Qh=0.0
	for k in range(0,c):
		Qh += ( S(V[k], V[k], W)/total_weight - ( (S(V[k], V_full ,W)/ total_weight) * (S(V[k], V_full ,W)/ total_weight) ) )
		# print "intermediate Qh=",Qh
	string = "Qh = "+str(Qh) +"\n"
	# print string
	# stats.write(str)
	return Qh
...
def M(i, V, W):
	templist=[]
	templist.append(i)
	return S(templist, V, W)


def newGNScore2(U,W,c):
	V = [None] * c
	for x in range(0,c):
		V[x] = []
	...
	for x in range(0,c):
		...
		for y in range(0,len(W)):
			for z in range(0,len(W)):
				i = y
				j = z
				Qh2 += ((W[i][j] - (M(i ,V_full, W) * M(j, V_full, W)/total_weight)) * U[x][i] * U[x][j])
				# print "x: ",x, "  i: ",i, " j: ",j, " Qh2: ",Qh2, " U[x][i]: ", U[x][i], " U[x][j]: ", U[x][j]
				t1 = M(i ,V_full, W)
				t2 = M(j ,V_full, W)
				# print  " (M(i ,V_full, W): " +  str(t1) + " (M(j ,V_full, W) " + str(t2)

	Qh2  = Qh2 / total_weight
	return Qh2

def newGNScore3(U,W,c):
	V = [None] * c
	for x in range(0,c):
		V[x] = []
	...
	m =  np.asarray(m_array, dtype=np.float64)
	m_transpose = m.transpose()
	tempMatrix = np.dot(m_transpose, m)/total_weight
	B = np.subtract(W, tempMatrix)
	Qg = np.trace( np.dot(np.dot(U, B), np.matrix.transpose(U) )) / total_weight
...
def foo(receivedMatrix, WeightMatrix, normalizedMatrix, clusteringType, numberOfClusters, labels, originalNumberOfClusters):
	...
	if clusteringType == "louvain":
		# numberOfClusters = 6
		numberOfClusters = originalNumberOfClusters;
	...
	for r in range(1,2):
		...
		if (clusteringType == "kmeans"):
			...
			Silcoef = metrics.silhouette_score(Wnorm, np.asarray(partition.values()))
			score = "Silhoette Score: " + str(Silcoef)
			statfile2_silhouette = str(Silcoef)
		...
		# print "returnMatrix: ", returnMatrix, " Weight Matrix receivedMatrix: ",W, " #communities: ", len(returnMatrix)
		Q = newGNScore(returnMatrix, W, len(returnMatrix))
		# Qcorrected = newGNScore2(returnMatrix, W, len(returnMatrix))
		Q2 = newGNScore2(returnMatrix, W, len(returnMatrix))
		Qcorrected = newGNScore3(returnMatrix, W, len(returnMatrix))
		print ("Qg : ", Q, " Q2: ",Q2," Qg3 : ", Qcorrected)
		...
		statfile3 = open("clusterstats_condensed2.txt",'a')
		...
		statfile3.write(clusteringType + ", " + str(numberOfClusters) + "," + str(Q) + "," + statfile2_silhouette + "," + statfile2_fpc + ","+statfile2_modularity+"\n" )
...
def similarity():
	...
	# DATA_DIR = "static/server_services/pythondata_hiervast2015"
	
	# DATA_DIR = "static/server_services/pythondata_usf"
	...
	# DATA_DIR = "static/server_services/pythondata_hierinfovis2015"
	DATA_DIR = "static/server_services/pythondata_test"
	...
	# print "fekete - manivannavn: ", X[12][33]
